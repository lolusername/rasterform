...
In the Image workspace, Still export offers two framing choices. **Current view** uses the active editor camera and the existing long-edge choices: 2K, 4K, or 8K with High. **Straight on** creates a fresh, front-aligned camera for that export and renders at the uploaded source image's exact original width and height without moving or replacing the editor camera. This changes only framing and output dimensions; it does not change the mesh, material, or render quality. High remains the existing 2× tiled renderer. Final and desktop Cycles Pro retain their current full-quality contracts and 4K maximum, so an original above 4K requires High with Straight on to export its exact pixels. Living Loop keeps its current camera, 2K/4K controls, and High 2× behavior.
...
The desktop edition also adds a true **Blender project** export beside the unchanged Exact GLB. Exact `.blend` keeps the source triangles and UVs. Balanced and Lightweight create a lower-density editable mesh, regenerate a padded `UVMap`, preserve the visible material/color attribute, and apply angle-aware smooth shading. Regular image reliefs are deterministically rebuilt as quad grids instead of being sent through a generic remesher; feature-aware sampling preserves narrow peaks and grooves that fall between the lower-density grid points. Irregular text/freeform geometry uses Blender's fixed-seed QuadriFlow path with sharp edges, open boundaries, and attributes protected. The saved project contains one selected mesh and no camera, light, floor, grid, or plane. Blender's viewport grid overlays are disabled in the project. The web edition retains Exact GLB because a browser cannot securely launch a local Blender executable.

Rasterform can export transparent 2K/4K/8K current-view still PNGs, straight-on stills at an image source's exact original dimensions, lossless 2K/4K Living Form frame sequences, height maps, reusable recipes, exact portable GLB geometry, editable Blender `.blend` projects on desktop, and watertight STL files when the topology is ready to fabricate.
